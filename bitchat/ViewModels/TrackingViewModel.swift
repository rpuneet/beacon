...
// bitchat
//
// UI state management for the tracking feature
//
...
/// ViewModel for tracking UI state management
@MainActor
final class TrackingViewModel: ObservableObject {
    ...
    var peersWithLocation: [PeerLocation] {
        trackingService.peerLocations.values
    ...
    var allPeerLocations: [PeerLocation] {
        Array(trackingService.peerLocations.values)
            .sorted { $0.timestamp > $1.timestamp }
    }
    ...
    var selectedPeerLocation: PeerLocation? {
        ...
        return trackingService.peerLocations[id]
    }

    /// Title for the ping button based on state
    var pingButtonTitle: String {
        switch trackingService.pingState {
        case .idle:
            return "Ping Friends"
    ...
    var isPinging: Bool {
        if case .pinging = trackingService.pingState { return true }
        return false
    }
    ...
    func pingAll() {
        trackingService.sendPing()
    }

    /// Send ping to a specific peer
    func pingPeer(_ peerID: PeerID) {
        trackingService.sendPing(to: [peerID])
    }
